...
百科生成方式：\`npm run docs:encyclopedia\`。鱼类、尺寸、动作、事件、成就、价格区间、装备概率与保底阈值从当前运行代码和正式数据生成。2026-09-08逐章核对；此文是对应游戏版本的快照，不承诺未来版本相同。
...
- 三窗口：桌宠、信息面板和结果卡；结果卡同时承载鱼获、特殊事件、随机脱钩和新成就提示，桌面鱼缸窗口不创建。
...
| \`resultWindow\` | 普通鱼获、特殊事件、脱钩和成就解锁卡 | 不支持拖边缩放；随桌宠大小设置有限缩放，优先放主体上方，顶部空间不足时改到下方并限制在工作区；每张统一10秒隐藏；同一收获解锁的成就排在该收获卡之后并逐张显示 |
...
收藏首页保留三个系列原封面和唯一发现进度；系列详情逐行显示图标、标题、介绍、首次发现时间和次数，未发现内容隐藏文字并灰阶。60项内容引用34张256×256图案与34套1536×256六帧动作条；漂流瓶共享四类徽记图案，科研和生态逐项独立。所有结果卡每张显示10秒后自动隐藏，不代表撤销已经入库的收获；若本次收获解锁成就，成就卡在收获卡结束后显示。结果卡默认168×176 DIP，宽度为round(clamp(168×桌宠缩放,152,200))，高度按22:21取整；不是跟随桌宠在全范围同比放大。
...
- 成就：一行一条，左侧高清图标、中间条件、右侧进度；固定字号和图标尺寸，支持系列/状态/搜索/排序、3项关注与补发摘要。新达成时复用结果卡显示系列、成就图标、名称、条件和“解锁新成就！”，每张保留10秒；多项按成就册顺序排队。
