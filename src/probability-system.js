(function exposeProbability(globalScope) {
  ...
  function chooseRarity(rng) { return weighted(rng, RARITIES.map((id) => ({ id, weight: RARITY_WEIGHTS[id] }))).id; }
  ...
  function variantWeights(counters = {}, variant = {}) {
    const base = { normal: 8800, alternate: 1000, golden: 170, iridescent: 30, ...(variant.weights || {}) };
  ...
  function encounter(rng, { habitat, unlockedPacks, fish, specialEvents = [], counters = {}, specialEventPity = null, collection = {}, variantConfig } = {}) {
    const events = eligibleSpecialEvents(specialEvents, habitat);
    const trace = { special: null, pack: null, rarity: null, fish: null, randomUnhook: null, variant: null };
    ...
    if (events.length) {
      ...
      if (due) {
        ...
        return { type: 'special', encounterType: 'special', eventId: event?.id || null, seriesId: event?.seriesId || null, event: event || null, probabilityVersion: PROBABILITY_VERSION, topLevelMode: 'scheduled_interval', selectionMode: forceNew ? 'new_item_pity' : 'weighted', nextSpecialEventPity, rollTrace: trace };
      }
    ...
    const rarityRoll = unit(streams ? streams.rarity : rng); const rarity = weighted(() => rarityRoll, RARITIES.map((id) => ({ id, weight: RARITY_WEIGHTS[id] }))).id;
    trace.rarity = { roll: rarityRoll, candidateCount: RARITIES.length, result: rarity };
    ...
    if (!selected) return { type: 'none', encounterType: 'none', probabilityVersion: PROBABILITY_VERSION, nextSpecialEventPity: normalizeSpecialPity(specialEventPity || {}), rollTrace: trace };
    ...
    if (randomUnhook) return { type: 'random_unhook', encounterType: 'fish', packId, rarity: selected.rarity, fishId: selected.id, randomUnhook: true, variant: null, probabilityVersion: PROBABILITY_VERSION, nextSpecialEventPity, rollTrace: trace };
    const variantRoll = unit(streams ? streams.variant : rng); const variant = rollVariant(() => variantRoll, counters, variantConfig); trace.variant = { roll: variantRoll, result: variant };
    return { type: 'fish', encounterType: 'fish', packId, rarity: selected.rarity, fishId: selected.id, randomUnhook: false, variant, probabilityVersion: PROBABILITY_VERSION, nextSpecialEventPity, rollTrace: trace };
  }
  const api = { RARITIES, RARITY_WEIGHTS, SPECIAL_RATE, SPECIAL_BASE_RATE_BP, SPECIAL_INTERVAL_MIN, SPECIAL_INTERVAL_MAX, PROBABILITY_VERSION, NON_SPECIAL_STREAK_LIMIT, DUPLICATE_SPECIAL_STREAK_LIMIT, RANDOM_UNHOOK_RATE, weighted, eligibleSpecialEvents, normalizeSpecialPity, chooseSpecialInterval, namedStreams, chooseSpecial, chooseRarity, choosePack, chooseFish, pityMinimum, variantWeights, rollVariant, updatePity, encounter };
